...
-- 4. 부호 판별
--    SIGN(A) : A가 양수이면 1, A가 음수이면 -1, A가 0이면 0을 반환한다.
SELECT
       SIGN(5)
     , SIGN(-5)
     , SIGN(0)
...
-- 8. 원하는 자릿수로 반올림
--   ROUND(A, [DIGIT]) : 실수 A를 DIGIT 자릿수로 반올린 값을 반환, DIGIT을 생략하면 정수로 반올린 값을 반환한다.
SELECT
...
--   TRUNC(A, [DIGIT]) : 실수 A를 DIGIT 자릿수로 절사한 값을 반환, DIGIT을 생략하면 정수로 절사한 값을 반환한다.
SELECT
