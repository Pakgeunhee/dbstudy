...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE POSITION IN (SELECT POSITION
                     ...
                     WHERE DEPART = 2);  -- 서브쿼리의 WHERE절에서 사용한 칼럼이 PK나 UNIQUE 칼럼이 아니므로 다중 행 서브쿼리이다.
                                         -- 단일 행 연산자인 등호(=) 대신 다중 행 연산자 IN을 사용해야한다.
...
SELECT 부서정보
  FROM 부서
 WHERE 부서번호 IN (직급이 '과장'인 사원들의 근무하는 부서번호)
*/
SELECT DEPT_NO, DEPT_NAME, LOCATION
  FROM DEPARTMENT_TBL
 WHERE DEPT_NO IN(SELECT DEPART
                    FROM EMPLOYEE_TBL
                   WHERE POSITION = '과장');  -- WHERE절에서 사용한 DEPART_NAME 칼럼은 PK/UNIQUE가 아니므로 다중 행 서브쿼리이다.
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAFBLAABAAALDJAAA';
