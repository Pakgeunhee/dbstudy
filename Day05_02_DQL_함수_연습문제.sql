...
SELECT COMMISSION_PCT, FIRST_NAME, LAST_NAME, SALARY, COMMISSION_PCT * SALARY AS 커미션
  FROM EMPLOYEES
...
SELECT JOB_ID, FIRST_NAME, LAST_NAME, MANAGER_ID
  FROM EMPLOYEES
...
SELECT DEPARTMENT_ID, FIRST_NAME, LAST_NAME, TO_CHAR(SALARY, '99,999')
  FROM EMPLOYEES
 WHERE DEPARTMENT_ID = '80'
 ORDER BY SALARY DESC;

-- 13. EMPLOYEES 테이블에서 전체 사원의 근무 개월 수를 정수로 조회하시오. 1개월 1일을 근무했다면 2개월을 근무한 것으로 처리해서 조회하시오. ??? 
SELECT FIRST_NAME, LAST_NAME, MONTH_BETWEEN(SYSDATE, TO_DATE(HIRE_DATE, 'YYYY-MM-DD'))
  FROM EMPLOYEES;
...
SELECT 
       PHONE_NUMBER LIKE '011%' AS 'MOBILE'
     , PHONE_NUMBER LIKE '515%' AS 'EAST'
     , PHONE_NUMBER LIKE '590%' AS 'WEST'
     , PHONE_NUMBER LIKE '603%' AS 'SOUTH'
     , PHONE_NUMBER LIKE '650%' AS 'NORTH'
  FROM EMPLOYEES;
