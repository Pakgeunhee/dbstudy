...
/*
    ...
    3. 칼럼 수정   : ALTER TBALE 테이블명 MODIFY 칼럼명 데이터타입 [제약조건] 
...
-- 3. BANK_TBL 테이블의 은행명(BANK_NAME) 칼럼의 데이터타이블 VARCHAR2(15 BYTE)으로 변경하시오.
ALTER TABLE BANK_TBL
    MODIFY BANK_NAME VARCHAR2(15 BYTE);
    
-- 4. CUSOMER_TBL 테이블에서 고객명(NAME) 칼럼의 이름을 CUST_NAME으로 변경하시오.
ALTER TABLE CUSTOMER_TBL
...
ALTER TABLE BANK_TBL
    MODIFY BANK_NAME VARCHAR(15 BYTE) NOT NULL; 
...
-- 9. CUSTOMER_TBL 테이블의 CUST_NAME 칼러의 NOT NULL 제약조건을 NULL 제약조건으로 변경하시오.
ALTER TABLE CUSTOMER_TBL
